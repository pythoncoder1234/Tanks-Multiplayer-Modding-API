...
public class Chunk implements Comparable<Chunk>
{
    ...
    public void addBorderFace(int side, Level l)
    {
        Face f = new Face(null,
                Math.max(0, Math.min(l.sizeX, (chunkX + x1[side]) * Chunk.chunkSize)) * tanks.Game.tile_size,
                Math.max(0, Math.min(l.sizeY, (chunkY + y1[side]) * Chunk.chunkSize)) * tanks.Game.tile_size,
                Math.max(0, Math.min(l.sizeX, (chunkX + x2[side]) * Chunk.chunkSize)) * tanks.Game.tile_size,
                Math.max(0, Math.min(l.sizeY, (chunkY + y2[side]) * Chunk.chunkSize)) * tanks.Game.tile_size,
    ...
    {
        return iterateOutwards((int) (posX / tanks.Game.tile_size), (int) (posY / tanks.Game.tile_size), radius);
    }
    ...
    {
        return chunkPos * Chunk.chunkSize * tanks.Game.tile_size;
    }
    ...
    {
        return getChunksInRange((int) (x1 / tanks.Game.tile_size), (int) (y1 / tanks.Game.tile_size),
                (int) (x2 / tanks.Game.tile_size), (int) (y2 / tanks.Game.tile_size));
    }
    ...
    public static Stream<Chunk> getChunksInRange(int tx1, int ty1, int tx2, int ty2)
    {
        ...
        return chunkList.stream().filter(chunk -> tanks.Game.lessThan(true, x1, chunk.chunkX, x2)
                && tanks.Game.lessThan(true, y1, chunk.chunkY, y2));
    }

    /** Expects all pixel coordinates. */
    public static Stream<Chunk> getChunksInRadius(double x1, double y1, double radius)
    {
        return getChunksInRadius((int) (x1 / tanks.Game.tile_size), (int) (y1 / tanks.Game.tile_size), (int) (radius / tanks.Game.tile_size));
    }
    ...
    {
        t.colR = l.colorR + (tanks.Game.fancyTerrain ? r.nextDouble() * l.colorVarR : 0);
        t.colG = l.colorG + (tanks.Game.fancyTerrain ? r.nextDouble() * l.colorVarG : 0);
        t.colB = l.colorB + (tanks.Game.fancyTerrain ? r.nextDouble() * l.colorVarB : 0);
        t.depth = tanks.Game.fancyTerrain && tanks.Game.enable3dBg ? r.nextDouble() * 10 : 0;
        return t;
    }
    ...
    {
        if (!tanks.Game.currentLevel.mapLoad && (posX < 0 || posX >= tanks.Game.currentSizeX * tanks.Game.tile_size || posY < 0 || posY >= tanks.Game.currentSizeY * tanks.Game.tile_size))
            return null;
    ...
    {
        return  (int) (a / tanks.Game.tile_size) % chunkSize;
    }
    ...
    public static void drawDebugStuff()
    {
        ...
        for (Chunk c : chunkList)
            Drawing.drawing.drawRect(addCoords(c.chunkX, chunkSize / 2.) * tanks.Game.tile_size, addCoords(c.chunkY, chunkSize / 2.) * tanks.Game.tile_size,
                    chunkSize * tanks.Game.tile_size, chunkSize * tanks.Game.tile_size, 2);
    }
    ...
    public static Chunk getChunk(double posX, double posY, boolean isTileCoords)
    {
        if (!isTileCoords)
        {
            posX = posX / tanks.Game.tile_size;
