...
public class EventTankTransformPreset extends PersonalEvent
{
    ...
    public void execute()
    {
        ...
        if (this.clientID == null && t instanceof TankRemote t1 && t1.tank instanceof TankAIControlled && (!revert || !t1.parentTransformations.isEmpty()))
        {
            TankAIControlled target;

            if (revert)
                target = t1.parentTransformations.remove(t1.parentTransformations.size() - 1);
            else
            {
                target = ((TankAIControlled) t1.tank).healthTransformTank;
                if (sight)
                    target = ((TankAIControlled) t1.tank).sightTransformTank;

                TankAIControlled p = new TankAIControlled(t1.tank.name, 0, 0, 0, 0, 0, 0, 0, TankAIControlled.ShootAI.none);
                ((TankAIControlled) t1.tank).cloneProperties(p);
                t1.parentTransformations.add(p);
            }
            ...
            ((TankAIControlled) t1.tank).spawnedTankEntries = target.spawnedTankEntries;
