...
public class TeamDeathmatch extends Minigame
{
    ...
    public static final String[] coolItems = new String[]{
            "[Death Ray,bullet_electric.png,5,0,10000,100000000,bullet,electric,none,3.125,5,5.0,5,500.0,20.0,0.0,false,0.0,1,30.0]\n",
            "[Totally normal mine,mine.png,5,0,20,50,mine,20.0,0.0,300.0,0.25,5,20.0,60.0,true]\n"
    };
    ...
    public void start()
    {
        ...
        if (Game.game.window.pressedKeys.contains(InputCodes.KEY_LEFT_SHIFT) || Math.random() < 0.1)
        {
            shopNPC.posX -= 50;

            TankNPC aCoolTank = new TankNPC("npc", 44, 13, Math.PI / 2, new TankNPC.MessageList("Hello! I'm just a passerby who sells toys.", "I sell very cool toys for very cheap prices.", "Why don't you give my toy a try?", TankNPC.shopCommand), "", 0, 0, 255);

            for (String s : coolItems)
                aCoolTank.shopItems.add(Item.parseItem(null, s));

            Game.addTank(aCoolTank);
            ModAPI.displayText(new FixedText(FixedText.types.actionbar, "A stranger has appeared on the bridge. Maybe you should go talk to him...", 500));
        }
